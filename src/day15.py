...
def move_robot(map, d, p):
    ...
    if map[i][j] == '#':
        ...
        if d[0] == 0:
            ...
            aff_obs = affected_obstacles(map, d, (i,j), '\]')
            if aff_obs:
...
def affected_obstacles(map, d, p, c):
    ...
    if not no_wall:
        ...
        if map[next_posli][next_poslj] == '\[':
            upper_check = affected_obstacles(map,d,(next_posli,next_poslj),'\[')
            if not upper_check:
                return res
            else:
        ...
        if map[next_posli][next_poslj] == '\]' and map[next_posri][next_posrj] == '\[':
            ...
            left_check = affected_obstacles(map, d, (next_posli, next_poslj), '\]')
            right_check = affected_obstacles(map, d, (next_posri, next_posrj), '\[')
            if left_check and right_check:
                res = [(r_i,r_j),(l_i,l_j)] + left_check + right_check
                return res
            else:
                return res
        elif map[next_posli][next_poslj] == '\]':
            ...
            if left_check:
                ...
                return res
        elif map[next_posri][next_posrj] == '\[':
